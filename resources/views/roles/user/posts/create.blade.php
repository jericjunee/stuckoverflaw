...
@section('content')
  <h2 class="text-5xl text-center">Create post</h2>
  ...
  <div class="flex justify-center mt-10">
    <form class="w-1/2" action="/dashboard/posts" method="POST" enctype="multipart/form-data">
      @csrf
      <div class="mb-6">
        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 ">Post Title</label>
        <input type="text" id="title" name="title" class="bg-light border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mb-5" placeholder="Enter title" required>

        <div class="flex flex-row items-center gap-5 text-sm">
          <label class="text-sm font-heading font-medium" for="category">Category: </label>
          <select class="font-body bg-light border border-gray-300 text-dark text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-2/5 p-2.5" name="category" id="category">
      ...
      <textarea id="description" name="description" rows="2" class="block mb-5 p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Write a short description.."></textarea>
          
      <div class="mb-4 w-full bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600 text-lg">
        ...
        <div class="py-2 px-4 bg-white rounded-b-lg dark:bg-gray-800">
            <textarea id="body" name="body" rows="8" class="block px-0 w-full text-sm text-gray-800 bg-white border-0 !outline-0 !outline-none focus:outline-0 focus:border-0 focus:border-none focus:outline-none" placeholder="Start writing your post..." required></textarea>
        </div>
      </div>

      <div class="mt-5">
        <label class="block mb-2 text-sm font-heading font-medium text-gray-900 dark:text-gray-300" for="image">Upload cover photo</label>
        <input class="block w-2/5 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 cursor-pointer" aria-describedby="file_input_help" id="image"  name="image" type="file">
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help">Only JPG, JPEG, PNG file extensions are accepted.</p>
      </div>
      
      <div class="flex justify-center mt-5">
        <input type="submit" value="Create post" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none">
      </div>
    </form>
  </div>
