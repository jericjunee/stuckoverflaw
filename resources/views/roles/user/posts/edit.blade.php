...
@section('content')
  <h2 class="text-5xl text-center">Edit post</h2>
  ...
  <div class="flex justify-center mt-10">
    <form class="w-1/2" action="/dashboard/posts/{{ $post->id }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="mb-6">
        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 ">Post Title</label>
        <input type="text" id="title" name="title" class="bg-light border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mb-5" value="{{ $post->title }}" required>

        <div class="flex flex-row items-center gap-5 text-sm">
          <label class="text-sm font-heading font-medium" for="category">Change Category: </label>
      ...
      <div class="mb-4 w-full bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600 text-lg">
        ...
        <div class="py-2 px-4 bg-white rounded-b-lg dark:bg-gray-800">
            <textarea id="body" name="body" rows="8" class="block px-0 w-full text-sm text-gray-800 bg-white border-0 !outline-0 !outline-none focus:outline-0 focus:border-0 focus:border-none focus:outline-none" required>{{ $post->body }}</textarea>
        </div>
      </div>

      <div class="mt-5">
        <label class="block mb-2 text-sm font-heading font-medium text-gray-900 dark:text-gray-300" for="image">Change cover photo</label>
