...
@section('content')
    <section class="flex flex-col gap-5 px-36 py-24">
      <h1 class="text-6xl mb-20 text-center">Edit post</h1>
        ...
        <form class="flex flex-col gap-10 font-body" action="/blog/{{ $post->id }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method("PUT")
          <input class="font-heading text-5xl font-bold !outline-none" type="text" name="title" id="title" value="{{ $post->title }}">
          <div class="flex flex-row items-center gap-5">
            <label class="text-lg font-heading font-medium" for="category">Edit category: </label>
            <select class="font-body bg-gray-50 border border-gray-300 text-dark text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-1/5 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="category" id="category">
          ...
          <textarea id="description" name="description" rows="2" class="block p-2.5 w-full text-lg text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Write a short description of your post..">{{ $post->description }}</textarea>
          ...
          <div class="mb-4 w-full bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600 text-lg">
            ...
            <div class="py-2 px-4 bg-white rounded-b-lg dark:bg-gray-800">
                <textarea id="body" name="body" rows="8" class="block px-0 w-full text-gray-800 bg-white dark:bg-gray-800 dark:text-white dark:placeholder-gray-400 !outline-none" placeholder="Start writing your post..." required>{{ $post->body }}</textarea>
            </div>
        </div>

        
        <div>
          <label class="block mb-2 text-xl font-heading font-medium text-gray-900 dark:text-gray-300" for="image">Upload new cover photo</label>
          <input class="block w-1/5 text-lg text-gray-900 bg-gray-50 rounded-lg border border-gray-300 cursor-pointer" aria-describedby="file_input_help" id="image"  name="image" type="file">
        ...
        <button type="submit" class="w-1/5 mt-10 text-white bg-blue-700 font-medium rounded-lg text-lg px-5 py-2.5 mr-2 mb-2 focus:outline-none">Update post</button>
